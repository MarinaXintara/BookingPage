...
public class RecommendationService {
    ...
    public List<RecommendedEventResponse> recommendForUser(Long userId) {
        ...
        List<EventVisit> visits = eventVisitRepository.findAll();
        List<Interaction> bookingHistory = bookingInteractions(bookings);
        List<Interaction> visitHistory = visitInteractions(visits);
        boolean hasBookingHistory = bookingHistory.stream().anyMatch(item -> item.userId().equals(userId));

        // A visitor without reservations is profiled strictly from attended events.
    ...
    private Set<Long> consumedEventIds(Long userId, List<Interaction> bookingHistory, List<Interaction> visitHistory) {
        ...
        bookingHistory.stream()
                ...
                .forEach(item -> eventIds.add(item.eventId()));
        visitHistory.stream()
                .filter(item -> item.userId().equals(userId))
                .forEach(item -> eventIds.add(item.eventId()));
        return eventIds;
    ...
    private List<IndexedInteraction> toIndexedInteractions(
        ...
        return interactions.stream()
                .map(interaction -> new IndexedInteraction(
                        userIndexes.get(interaction.userId()),
                        eventIndexes.get(interaction.eventId()),
                        interaction.value()))
                .toList();
    }
